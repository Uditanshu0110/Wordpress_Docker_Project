...
if ( ! class_exists( 'PixelgradeAssistant_Install_Notice' ) ) {
	...
	class PixelgradeAssistant_Install_Notice {
		...
		public function outputMarkup() {
			...
			$button_text = sprintf( esc_html__( 'Yes, install the %s plugin for free', 'rosa2-lite' ), 'Pixelgrade Assistant' );
			...
			if ( ! class_exists( 'PixelgradeAssistant' ) && file_exists( WP_PLUGIN_DIR . '/pixelgrade-assistant/pixelgrade-assistant.php' ) ) {
				/* translators: %s: Pixelgrade Assistant  */
				$button_text = sprintf( esc_html__( 'Yes, activate the %s plugin', 'rosa2-lite' ), 'Pixelgrade Assistant' );
			} ?>
				...
				<form class="pixassist-notice-form"
					...
					if ( $screenshot ) { ?>
						<img class="pixassist-notice__screenshot" src="<?php echo esc_url( $screenshot ); ?>"
						     width="1200" height="900" alt="<?php esc_html_e( 'Theme screenshot', 'rosa2-lite' ); ?>">
					<?php } ?>
					<div class="pixassist-notice__body">
						<h2><?php
							/* translators: %s: Theme name */
							echo wp_kses( sprintf( __( '%s is active! Are you looking for a better experience to set up your site?', 'rosa2-lite' ), $theme->get( 'Name' ) ), wp_kses_allowed_html( 'post' ) ); ?></h2>
						<p><?php
							/* translators: %s: Pixelgrade Assistant  */
							echo wp_kses( sprintf( __( 'We\'ve prepared a unique <strong>onboarding process</strong> through our %s plugin. It helps you get started and configure your upcoming website with ease. </strong> Let\'s make it shine!', 'rosa2-lite' ), 'Pixelgrade Assistant' ), wp_kses_allowed_html( 'post' ) ); ?></p>
						...
						<button type="submit" class="button dismiss"><span
								class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'rosa2-lite' ); ?></span><?php esc_html_e( 'No thanks, I\'ll be fine', 'rosa2-lite' ); ?>
						</button>
					</div>
		...
		public function outputJS() {
			...
			wp_localize_script( 'pixassist_notice_js', 'pixassistNotice', array(
				...
				'i18n'              => array(
					/* translators: %s: Pixelgrade Assistant  */
					'btnInstall'                => sprintf( esc_html__( 'Install the %s plugin', 'rosa2-lite' ), 'Pixelgrade Assistant' ),
					/* translators: %s: Pixelgrade Assistant  */
					'btnInstalling'             => sprintf( esc_html__( 'Installing the %s plugin from WordPress.org…', 'rosa2-lite' ), 'Pixelgrade Assistant' ),
					/* translators: %s: Pixelgrade Assistant  */
					'btnActivating'             => sprintf( esc_html__( 'Activating the %s plugin…', 'rosa2-lite' ), 'Pixelgrade Assistant' ),
					'btnGoToSetup'              => esc_html__( 'Ready! Start the Site Setup →', 'rosa2-lite' ),
					'btnError'                  => esc_html__( 'Please refresh the page 🙏 and try again…', 'rosa2-lite' ),
					'installedSuccessfully'     => esc_html__( 'The plugin was installed successfully.', 'rosa2-lite' ),
					'activatedSuccessfully'     => esc_html__( 'The plugin was activated successfully.', 'rosa2-lite' ),
					/* translators: %s: Pixelgrade Assistant  */
					'clickStartTheSiteSetup'    => sprintf( esc_html__( 'Click to start the onboarding process provided by %s.', 'rosa2-lite' ), 'Pixelgrade Assistant' ),
					'error'                     => esc_html__( 'We are truly sorry 😢 Something went wrong and we couldn\'t make sense of it and continue with the plugin setup.', 'rosa2-lite' ),
					// TGMPA strings to look for.
					'tgmpActivatedSuccessfully' => esc_html__( 'The following plugin was activated successfully:', 'rosa2-lite' ),
					'tgmpPluginActivated'       => esc_html__( 'Plugin activated successfully.', 'rosa2-lite' ),
					'tgmpPluginAlreadyActive'   => esc_html__( 'No action taken. Plugin was already active.', 'rosa2-lite' ),
					'tgmpNotAllowed'            => esc_html__( 'Sorry, you are not allowed to access this page.', 'rosa2-lite' ),
				),
			) );
		}
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'You should not do that!', 'rosa2-lite' ), null );
		}
